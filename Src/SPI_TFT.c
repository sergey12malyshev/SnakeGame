...
void LCD_Fill(uint16_t color) {
	...
	for (n = 0; n < 640; n+=2) {buf[n]=i; buf[n+1]=j;}
...
void LCD_fillRect(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h, uint16_t color) {
	...
	else {HAL_SPI_Transmit_DMA(&hspi1, buf, nnum); for (t = 0; t < nnum*3+nnum/14; t++) {__NOP();}}
...
void simple_font_OUT(uint8_t ascii, uint16_t x0, uint16_t y0, uint16_t size, uint16_t fgcolor, uint16_t bgcolor)	
{
	...
	for (i =0; i<8; i++ )
	{
		...
		for(f=0;f<8;f++)
		{
			...
			else {LCD_fillRect(x0+i*size, y0+f*size, size, size, bgcolor);}
		}
	}
}
//
...
void NUM_OUT(uint8_t ascii, uint16_t x0, uint16_t y0, uint16_t size, uint16_t fgcolor, uint16_t bgcolor)	
{
	...
	if (H%8 != 0) {H=((H/8)+1)*8;} // если делится без остатка, то все нормально, если нет, то округляем до значений байт, то есть до кратного 8 числа (в массиве все равно байты)
...
void FONT_OUT(uint8_t ascii, uint16_t x0, uint16_t y0, uint16_t size, uint16_t fgcolor, uint16_t bgcolor)	
{
	...
	if (ascii == 32) { // пробел, который не входит в массив шрифта
